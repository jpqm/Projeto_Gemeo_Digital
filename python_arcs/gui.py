...
class UIUpdater(QObject):
    """Classe de sinais para atualização segura do PyQt a partir de threads secundárias."""
...
class RobotGUI(QMainWindow):
    ...
    def __init__(self, controller):
        ...
        self.updater.update_status.connect(self.set_status)
        self.updater.update_plot.connect(self.atualizar_grafico)
    ...
    def _criar_painel_esquerdo(self) -> QWidget:
        ...
        btn_abrir.clicked.connect(lambda: self._obter_serial().send("M97 B60 T0.2"))
        left_layout.addWidget(btn_abrir)

        btn_fechar = QPushButton("Fechar Garra")
        btn_fechar.clicked.connect(lambda: self._obter_serial().send("M97 B0 T0.2"))
    ...
    def _obter_unity(self):
        """Retorna a referência do cliente Unity independente do nome da variável."""
        return getattr(self.controller, 'unity_cli', getattr(self.controller, 'unity', None))

    def _obter_serial(self):
        """Retorna a referência do driver Serial independente do nome da variável."""
        return getattr(self.controller, 'serial_drv', getattr(self.controller, 'serial', None))
    ...
    def verificar_botao_unity(self):
        ...
        unity = self._obter_unity()
        serial = self._obter_serial()
    ...
    def _executar_movimento(self, valores):
        ...
        self.updater.update_plot.emit(list(x), list(y), list(z), True)
        self.em_movimento.clear()
        self.updater.update_status.emit("Movimento concluído!")

    def home(self):
        """Retorna o robô para a posição inicial (Home)."""
        def _go_home():
            self.em_movimento.set()
            self.updater.update_status.emit("Retornando ao Home...")
            x, y, z = self.controller.home()
            self.updater.update_plot.emit(list(x), list(y), list(z), True)
            self.em_movimento.clear()
            self.updater.update_status.emit("Aguardando comando...")
    ...
    def executar_rotina_lapis(self):
        ...
        def _disparar():
            self.updater.update_status.emit("Executando Pick & Place...")
            self.controller.rotina_lapis_suporte(plot_callback=self.updater.update_plot.emit)
            self.updater.update_status.emit("Rotina concluída!")
